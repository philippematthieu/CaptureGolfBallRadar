...
function comet3d2(varargin)
    ...
    for l=1:m
        xpoly([],[]);taill(l)=gce();
        taill(l).foreground=c(l);
        xpoly([],[]);body(l)=gce();
        body(l).foreground=c(l); body(l).thickness=2;
        xpoly([],[],"marks");headd(l)=gce();
        headd(l).mark_size_unit="point";
        headd(l).mark_size=6;
        headd(l).mark_style=9;
        headd(l).mark_foreground=c(l);
    end
    show_window();
    ...
    function anim()
        ...
        for i=1:n
            for l=1:m
                headd(l).data=[x(i,l),y(i,l),z(i,l)];
