...
\fancyhead[RO,RE]{ Golf Simulator in Scilab v0.1}
\fancyhead[CO,CE]{\textcolor{red}{free}}
\fancyhead[LO,LE]{\includegraphics[width=0.5cm]{Logo-golfball.jpg}}
\fancyfoot[LO,CE]{\fontsize{8}{11}\selectfont\textit{\begin{center}This document and the information contained herein are the property of Matthieu PHILIPPE.\\They must not be copied or disclosed to third parties without prior written authorization.\end{center}}\begin{flushright} \end{flushright}}
}
...
\renewcommand{\headrulewidth}{1pt}
\fancyhead[LO,LE]{\includegraphics[width=0.5cm]{Logo-golfball.jpg}}
\fancyhead[RO,RE]{Page \thepage\  / \pageref{LastPage} }
\fancyhead[CO,CE]{\textcolor{red}{free}}
\fancyfoot[LO,CE]{\fontsize{8}{11}\selectfont\textit{\begin{center}This document and the information contained herein are the property of Matthieu PHILIPPE.\\They must not be copied or disclosed to third parties without prior written authorization.\end{center}}\begin{flushright}-\end{flushright}}
}
...
The radar signal is recorded in WAV format and filtered to extract speed and spin information during flight, using power spectral density.
...
Of course, the academic literature and publications were quite abundant. But I came accross articles \cite{PWBJKH76} written in 1976 and \cite{SASDR94} in 1994. The description of the ball's physics pushed me to undertake modeling this system in Scilab (fig \ref{fig:hmisimulation}). Subsequently, I did it in Java, C++, and then C\# to use it under \emph{Unity game engine}{\textregistered \texttrademark}.


\begin{figure} [H]\centering
\includegraphics[width=10cm,keepaspectratio, ]{../sci/img/Pasted image 20250120171635.png}
\caption{HMI Of Simulation}
\label{fig:hmisimulation}
\end{figure}
...
I didn't know what I was getting into, but after a updating my knowledge on signal filtering and a few lines of programming in Scilab, I found a way to capture the essential data. Namely the speed and backspin of the flight of a golf ball (figure \ref{fig:radarcapture}).

\begin{figure} [H]\centering
\includegraphics[width=8cm,keepaspectratio, ]{../sci/img/Pasted image 20250122083320.png}
\caption{Radar Captures}
\label{fig:radarcapture}
\end{figure}
...
-\frac{1}{2}.\rho(T).C_d.S.v_{i}^2 \\
-\frac{1}{2}.\rho(T).C_d.S.v_{j}^2 \\
-\frac{1}{2}.\rho(T).C_d.S.v_{k}^2
\end{pmatrix}
\end{equation}
}
...
\end{eqnarray}
My own $C_{l1}$ from Club Mizuno JPX and Decathlon's ball are given as example in Table \ref{eq:mycl1}.

\begin{table}[h!]
\centering
\begin{tabular}{|c|c|}
\hline
\textbf{Club} & \textbf{$C_{l1}$} \\
\hline
D & 0.64 \\
W5 & 0.65 \\
H3 & 0.54 \\
...
The gravity Force expression is simply,
...
Due to the texture we have a loss in energy, that can be taken into account throught the Coefﬁcient Of Restitution \emph{e} (COR \cite{BLSJ94}).
...
\psi_{bo}&=&\theta + \arctan\left(\frac{V_{bfp}}{V_{bfn}}\right)\\
...
\omega_{bo}&=&Spin\ of\ the\ golf\ ball\\
I&=&Inertia\ (\ref{eq:massdiameterintertia})
\end{eqnarray*}

\section{Balls Velocity with Miss sweet spot}
It is difficult to perfectly center the ball and the club. The ball velocity model is modified to include a \emph{Miss} parameter. It is the amount by which the sweet spot is missed, the initial ball velocity in this case is (\ref{eq.miss}),
...
Due to the Club Path ($\alpha_{cp}$) and the Face to Path and Face Angle ($\gamma_{fp}$), the initial ball velocity and spin are modified.
...
Ainsi :
\[
V_{b0}\cdot\overrightarrow{i} = V_{bi} \cdot (1 - 0.3556 \cdot miss) 
\cdot \cos(\theta + \arctan\left(\frac{\sin(\theta) \cdot (1 + \frac{m}{M})}{(1 + e) \cdot \cos(\theta) \cdot (1 + \frac{m}{M} + \frac{5}{2})}\right)
\]

\[
V_{b0}\cdot\overrightarrow{j} = V_{bi} \cdot (1 - 0.3556 \cdot miss) 
\cdot \sin(\theta + \arctan\left(\frac{\sin(\theta) \cdot (1 + \frac{m}{M})}{(1 + e) \cdot \cos(\theta) \cdot (1 + \frac{m}{M} + \frac{5}{2})}\right)
\]
\[
V_{b0}\cdot\overrightarrow{k}=0 
\]

Le spin sur $\overrightarrow{i}$ n'est pas une composante dans l'axe du chemin de club. Le spin est applique sur le plan du sol, SideSpin  est dans l'axe de la face de de club sur $\overrightarrow{j}$, et le BackSpin dans l'axe du chemin de club $\overrightarrow{k}$
\[
w_{\overrightarrow{i}} = 0
\]

\[
w_{\overrightarrow{j}} = V_{b0}\overrightarrow{j} \cdot sin(\alpha_{cp} - \gamma_{fp}) \cdot Sac.coeffSpinLift(Sac.Type == Club)
\]

\[
w_{\overrightarrow{k}} = V_{b0}\overrightarrow{i} \cdot sin(\theta)  \cdot Sac.coeffSpin(Sac.Type == Club)
\]

\section{Complete Model of flight}


\section{External Function for the Ordinary Differential Equation solver}

\chapter{Bouncing and Rolling}
...
\begin{figure} [H]\centering
\includegraphics[width=6cm,keepaspectratio, ]{../sci/img/Pasted image 20250123184355.png}
\caption{The suitecase}
\label{fig:hw1}
\end{figure}


\begin{figure} [H]\centering
\includegraphics[width=8cm,keepaspectratio, ]{../sci/img/20250123_132304.jpg}

\includegraphics[width=4cm,keepaspectratio, ]{../sci/img/20250123_132310.jpg}
\includegraphics[width=4cm,keepaspectratio, ]{../sci/img/20250123_132345.jpg}
\caption{Internal with Arduino, Radar, Amplifier, Audio Boards}
\label{fig:hw2}
\end{figure}


\begin{figure} [H]\centering
\includegraphics[width=8cm,keepaspectratio, ]{../sci/img/IMG_2427.jpg}
\caption{Tiny Arduino}
\label{fig:hw5}
\end{figure}


\begin{figure} [H]\centering
\includegraphics[width=5cm,keepaspectratio, ]{../sci/img/IMG_2428.jpg}
\caption{Audio Board}
\label{fig:hw6}
\end{figure}

\begin{figure} [H]\centering
\includegraphics[width=5cm,keepaspectratio, ]{../sci/img/PCB_Back_s.jpg}
\caption{Audio Amplifier Board}
\label{fig:hw8}
\end{figure}

\begin{figure} [H]\centering
\includegraphics[width=8cm,keepaspectratio, ]{../sci/img/IMG_2429.png}
\caption{Radar Board}
\label{fig:hw7}
\end{figure}
...
\chapter{Data Anlysis}

\section{Filtered Radar Captures}

\begin{figure} [H]\centering
\includegraphics[width=8cm,keepaspectratio, ]{../sci/img/Pasted image 20250122083320.png}
\caption{Wav Radar Captures and its echo}
\label{fig:wavradarcapture}
\end{figure}


\subsection{Filtering}
\begin{figure} [H]\centering
\includegraphics[width=8cm,keepaspectratio, ]{../sci/img/Pasted image 20250122082849.png}
\caption{Captured Raw Data Wav}
\label{fig:rawcapture}
\end{figure}

\begin{figure} [H]\centering
\includegraphics[width=8cm,keepaspectratio, ]{../sci/img/Pasted image 20250122082913.png}
\caption{FFT of Filtered Raw Data}
\label{fig:rawfft}
\end{figure}

\subsection{Final Result}
\begin{figure} [H]\centering
\includegraphics[width=8cm,keepaspectratio, ]{../sci/img/Pasted image 20250120172502.png}
\caption{My power density Algorithm Results to identify Speed and BackSpin}
\label{fig:radarcapture}
\end{figure}

\begin{figure} [H]\centering
\includegraphics[width=8cm,keepaspectratio, ]{../sci/img/Pasted image 20250122082958.png}
\caption{Results of normalized mean filtered power spectral density}
\label{fig:filtered power density results}
\end{figure}

\begin{figure} [H]\centering
\includegraphics[width=12cm,keepaspectratio, ]{../sci/img/Pasted image 20250122083851.png}
\caption{Results of the filtered Density and caracteristics identification}
\label{fig:identifycarac}
\end{figure}

\begin{figure} [H]\centering
\includegraphics[width=8cm,keepaspectratio, ]{../sci/img/Pasted image 20250217094457.png}
\caption{Speed and Backspin echos}
\label{fig:identifycarac}
\end{figure}
